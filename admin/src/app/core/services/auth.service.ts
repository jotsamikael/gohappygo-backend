...
import { AuthService as ApiAuthService } from '../../gohappygobackend/services';
import { LoginResponseDto } from '../../gohappygobackend/models';
...
export class AuthenticationService {
  ...
  login(credentials: LoginRequest): Observable<LoginResponseDto> {
    return this.apiAuthService.authControllerLogin({
      body: credentials
    }).pipe(
  ...
  /**
   * Refresh access token. Logout on failure is handled by AuthInterceptor.
   */
  refreshToken(): Observable<{ access_token: string; message?: string }> {
    const refreshToken = this.authStorageService.getRefreshToken();
    if (!refreshToken) {
      return throwError(() => new Error('No refresh token available'));
    }

    return this.apiAuthService.authControllerRefreshToken({
      body: { refreshToken }
    }).pipe(
      tap((response) => {
        if (response.access_token) {
          this.authStorageService.updateAccessToken(response.access_token);
        }
      })
    );
  }
